...
def plot_stars(ax, year1=2015):
    ...
    now = datetime(datetime.now().year, 
                   datetime.now().month, 
                   datetime.now().day, tzinfo=tzinfo)
    bins = []
    counts = []
    for year in years:
    ...
    plt.setp(ax.get_yticklabels(), rotation=30,
            fontsize=10, fontproperties=lato, alpha=0.75)
    ax.set_xticks(matplotlib.dates.date2num([
...
def plot_coffee(ax, year1=2015):
    # avg 2 cups per day = 0.0625 * 2 gal / day
    # that's 45.6 gal / year
    x = np.linspace(year1, datetime.now().year + 1, 30)
    np.random.seed(13)
    y = np.cumsum(np.abs(np.random.randn(len(x))))
    y *= 45.6 * (x.max() - x.min()) / y.max()
    ax.plot(x, y, '.', color='C1' , ms=3)
    ax.plot(x, y, '-', color='C1', lw=3, alpha=0.5, label='gallons')
    plt.setp(ax.get_xticklabels(), rotation=30,
            fontsize=10, fontproperties=lato, alpha=0.75)
    plt.setp(ax.get_yticklabels(), rotation=30,
            fontsize=10, fontproperties=lato, alpha=0.75)
    ...
    for tick in ax.get_xticklabels() + ax.get_yticklabels():
        tick.set_fontsize(10)
    ax.legend(loc="upper left", fontsize=8)
    ax.set_ylabel("coffee", fontsize=16)
    ax.set_xlabel("year", fontsize=16)
